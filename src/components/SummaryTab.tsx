...
export default function SummaryTab() {
  ...
  const filtered = useMemo(() =>
    logs.filter(l => isAfter(l.timestamp, cutoff)),
    [logs, cutoff]
  );
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(generateClinicianSummary());
  };
  ...
  return (
    <div className="animate-slide-up space-y-4 pb-4">
      ...
      <div className="bg-card rounded-xl border p-4">
        <h3 className="text-sm font-semibold mb-4">Symptoms Over Time</h3>
        <div className="h-64">
      ...
      {recentAvg && (
        <div className="bg-card rounded-xl border p-4">
          <h3 className="text-sm font-semibold mb-3">This Period vs Baseline</h3>
          <div className="space-y-2">
            {[
              { label: 'Sleep', current: recentAvg.sleep, base: baseline.sleep, unit: '', invert: false },
              { label: 'Pain', current: recentAvg.pain, base: baseline.pain, unit: '', invert: true },
              { label: 'Mobility', current: recentAvg.mobility, base: baseline.mobility, unit: '', invert: false },
              { label: 'Intake', current: recentAvg.intake, base: baseline.intake, unit: '', invert: false },
              { label: 'Mood', current: recentAvg.mood, base: baseline.mood, unit: '', invert: false },
            ].map(item => {
              ...
              return (
                <div key={item.label} className="flex items-center justify-between py-1.5">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-muted-foreground">baseline {item.base}</span>
                    <span className={`text-sm font-semibold ${concern ? 'text-mode-red' : 'text-primary'}`}>
                      {item.current}
                      {diff !== 0 && (
      ...
      <div className="bg-card rounded-xl border overflow-hidden">
        ...
        {showExport && (
          <div className="px-4 pb-4 space-y-3 border-t">
            <pre className="text-xs bg-secondary/50 rounded-lg p-3 overflow-auto max-h-64 whitespace-pre-wrap mt-3 text-foreground/80">
              {generateClinicianSummary()}
            </pre>
            <div className="flex gap-2">
              <button
                onClick={copyToClipboard}
                className="flex-1 flex items-center justify-center gap-1.5 bg-primary text-primary-foreground rounded-lg py-2.5 text-sm font-medium hover:opacity-90 transition-opacity"
              >
                <Copy size={14} /> Copy Message
              </button>
              <button className="flex items-center justify-center gap-1.5 bg-secondary text-secondary-foreground rounded-lg px-4 py-2.5 text-sm font-medium hover:opacity-90 transition-opacity">
                <Download size={14} />
              </button>
            </div>
